...
namespace Code.Gameplay.Common.Physics
{
    public class PhysicsService : IPhysicsService
    {
        ...
        [Obsolete("Obsolete")]
        public IEnumerable<GameEntity> CircleCast(Vector3 position, float radius,
            int layerMask)
        {
            ...
            {
                GameEntity entity =
                    _collisionRegistry.Get<GameEntity>(OverlapHits[i].GetInstanceID());
                if (entity == null)
                    continue;
        ...
        [Obsolete("Obsolete")]
        public int CircleCastNonAlloc(Vector3 position, float radius, int layerMask,
            GameEntity[] hitBuffer)
        {
            int hitCount = OverlapCircle(position, radius, OverlapHits, layerMask);
